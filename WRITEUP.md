...
Wearable health monitors face a fundamental tension: to catch genuine cardiac anomalies, they need sensitive alert thresholds, but sensitive thresholds produce floods of false alarms during normal exercise. A fitness tracker that alerts every time heart rate exceeds 90 BPM will fire constantly during a jog — training users to ignore it entirely.
...
We chose the **PPG-DaLiA dataset** (wrist-worn photoplethysmography + accelerometer data from 15 subjects across 8 activities) because it captures exactly this tension: real physiological signals during both exercise and rest, where simple thresholds demonstrably fail.
...
We preprocess by synchronizing to 32 Hz, computing accelerometer magnitude, and creating 30-second rolling windows with 5-second stride. Each window becomes a data point: `(960 samples x 5 features)`.

### Three Detection Methods

**1. Heart Rate Threshold (baseline):**
Estimate heart rate via PPG peak detection, alert when HR exceeds a fixed threshold. Simple but context-blind — any threshold faces an inherent tradeoff between catching rest anomalies and triggering exercise false alarms.

**2. Isolation Forest (statistical baseline):**
Extract summary statistics per window (mean, std, min, max per channel), fit an Isolation Forest on exercise windows to learn "normal" patterns. Better than thresholds but limited to hand-crafted features that can't capture temporal signal relationships.
...
Threshold detection faces an irreconcilable tradeoff between sensitivity and specificity:
...
Wood Wide achieves comparable rest detection (9.2% vs 10.6%) with **5x fewer exercise false alarms** (5.1% vs 25.4%). It breaks the threshold tradeoff by understanding that high heart rate during exercise is normal.
...
Notable: stairs and walking have near-zero false positive rates. Cycling is higher (11%) — likely because it's a steady-state exercise with less movement variation than walking.

## Limitations

1. **Sitting false alerts (35-48%):** The centroid-based approach flags sitting windows at a high rate. This is because "sitting" embeddings are distant from the exercise centroid by design — the detector conflates "different from exercise" with "anomalous." A multi-centroid approach (one per activity) would address this.
...
5. **PPG signal quality:** Wrist-worn PPG is noisy, especially during movement. Heart rate estimates from peak detection are approximate. The embedding approach is more robust to this noise since it operates on statistical features rather than individual peaks.
